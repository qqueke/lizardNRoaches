...
void *thread_control_wasps()
{
    int size;

    srandom(time(NULL));

    // Generate a random number of waspes
    int num_wasps = random() % 10 + 1;
    // int *wasps_digit = (int *)malloc(num_wasps * sizeof(int));

    // for (int i = 0; i < num_wasps; ++i)
    //     wasps_digit[i] = random() % 5 + 1;

    // Send connection message
    ClientType client_type = CLIENT_TYPE__WASP;
    ...
    size = zmq_send_WaspConnectReq(requester, num_wasps);
    verify_status(context, requester, size);
    ...
    if ((int)resp->n_bot_id != num_wasps)
        num_wasps = resp->n_bot_id;
    // {
    //     num_wasps = resp->n_bot_id;
    //     int *temp_wasps_digit = (int *)realloc(wasps_digit, num_wasps * sizeof(int));
    //     if (temp_wasps_digit != NULL)
    //         wasps_digit = temp_wasps_digit;
    // }

    int wasps_id[num_wasps];
    for (int i = 0; i < num_wasps; ++i)
    {
        wasps_id[i] = resp->bot_id[i];
    }
    ...
    while (!stop)
    {
        ...
        if (size == -1)
            continue;
        size = zmq_send_BotMovementReq(requester, wasps_id[index], token, direction, client_id);
        if (size == -1)
            continue;
        ...
        if (resp->resp == RESPONSE__SUCCESS)
        {
            ...
            mvprintw(index, 0, "%d Wasp %d: moved %s", counter, index, move);
...
int main(int argc, char *argv[])
{
    ...
    pthread_create(&worker_id[0], NULL, thread_control_wasps, NULL);
