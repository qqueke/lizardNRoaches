  ...
  while (1) {
    ...
    if (client_type == CLIENT_TYPE__LIZARD) {
      ...
      if (msg_type == MESSAGE_TYPE__CONNECT) {
        ...
        // Send to all the displays a snapshot (Ignore on the ones that do not
        // need or just process it?)
        type = GAME_SNAPSHOT;
      ...
      // Lizzard movement message
        ...
        int roaches_digit =
            eat_roaches(board, list_index, roaches, dead_roaches_list);
...
void *thread_bots(void *ptr) {
  ...
  while (1) {
    ...
    Timeout_Roaches(pusher, board, roach_clients, roaches, dead_roaches_list,
                    &n_roaches);
    ...
    if (client_type == CLIENT_TYPE__ROACH) {
      ...
      if (msg_type == MESSAGE_TYPE__CONNECT) {
        ...
        int temp_num_roaches =
            ((int)req->n_roach_digit < (MAX_BOTS - _n_roaches - _n_wasps))
                ? (int)req->n_roach_digit
                : (MAX_BOTS - _n_roaches - _n_wasps);
        l = pthread_mutex_lock(&num_roaches_mtx);
        if (l != 0)
          exit(EXIT_FAILURE);
        ...
        for (int i = 0; i < temp_num_roaches; i++) {
          ...
          roach = create_roach(board, roach_client->id, roach_id,
                               req->roach_digit[i]);
        ...
        size = zmq_send_BotConnectResp(responder, temp_roaches_id,
                                       temp_num_roaches, temp_token,
                                       temp_client_id);
        if (size == -1)
          continue;
      }
      ...
      // Roach_t disconnect
        ...
        remove_roaches_and_dead_by_client_id_lf(roaches, dead_roaches_list,
                                                &n_roaches, roach_client->id);
      ...
      if (msg_type == MESSAGE_TYPE__CONNECT) {
        ...
        int temp_num_wasps =
            ((int)req->num_wasps < (MAX_BOTS - _n_roaches - _n_wasps))
                ? (int)req->num_wasps
                : (MAX_BOTS - _n_roaches - _n_wasps);
        l = pthread_mutex_lock(&num_wasps_mtx);
        if (l != 0)
        ...
        size = zmq_send_BotConnectResp(responder, temp_wasps_id, temp_num_wasps,
                                       temp_token, temp_client_id);
        if (size == -1)
          continue;
      }
      // Wasp_t movement
...
void *thread_publish(void *ptr) {
  ...
  while (1) {
    ...
    if (type == INCREMENTAL_UPDATE) {
      ...
      size = zmq_send(publisher, &game_state,
                      (WINDOW_SIZE * WINDOW_SIZE) * sizeof(char), 0);
      if (size == -1)
        continue;
      ...
      for (int i = 1; i < SCORES_WIN_HEIGHT; ++i) {
        ...
        wclrtoeol(score_win);   // Clear from the current position to the end of
                                // the line
      }
